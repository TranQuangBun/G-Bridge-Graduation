...
export async function getAllSavedJobs(req, res) {
  try {
    ...
    // Format the data to include full job information
    const formattedJobs = data.savedJobs
      .filter((savedItem) => savedItem.job) // Filter out null jobs
      .map((savedItem) => ({
        id: savedItem.job.id,
        title: savedItem.job.title,
        province: savedItem.job.province,
        commune: savedItem.job.commune,
        address: savedItem.job.address,
        expirationDate: savedItem.job.expirationDate,
        minSalary: savedItem.job.minSalary,
        maxSalary: savedItem.job.maxSalary,
        salaryType: savedItem.job.salaryType,
        statusOpenStop: savedItem.job.statusOpenStop,
        savedDate: savedItem.savedDate,
        organization: savedItem.job.organization
          ? {
              id: savedItem.job.organization.id,
              name: savedItem.job.organization.name,
              logo: savedItem.job.organization.logo,
            }
          : null,
        workingMode: savedItem.job.workingMode
          ? {
              id: savedItem.job.workingMode.id,
              name: savedItem.job.workingMode.name,
            }
          : null,
        domains: savedItem.job.domains
          ? savedItem.job.domains.map((jd) => ({
              id: jd.domain?.id,
              name: jd.domain?.name,
            }))
          : [],
        requiredLanguages: savedItem.job.requiredLanguages
          ? savedItem.job.requiredLanguages.map((jl) => ({
              id: jl.language?.id,
              name: jl.language?.name,
              level: jl.level,
            }))
          : [],
      }));
    ...
    return sendSuccess(res, formattedJobs, "Saved jobs fetched successfully");
  } catch (error) {
