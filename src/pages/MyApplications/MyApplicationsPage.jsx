...
const MOCK_APPLICATIONS = [
  {
    ...
    status: "Under Review",
    description: "Leading global language services company seeking experienced interpreter for high-level conferences and business meetings.",
    requirements: ["5+ years conference interpreting experience", "Professional certification", "Fluent English and Vietnamese"]
  },
  {
    id: 2,
    company: "MedLingua",
    ...
    status: "Shortlisted",
    description: "Healthcare interpretation services for Vietnamese-speaking patients in medical settings.",
    requirements: ["Medical terminology knowledge", "Healthcare interpreting certification", "Compassionate communication skills"]
  },
  {
    id: 3,
    company: "EduBridge",
    ...
    status: "Rejected",
    description: "Translate educational materials and online courses from English to Vietnamese for K-12 students.",
    requirements: ["Education background preferred", "Translation experience", "Understanding of pedagogical concepts"]
  },
  {
    id: 4,
    ...
    description: "Translate technical documentation, software interfaces, and user manuals for technology companies.",
    requirements: ["Technical translation experience", "Software localization knowledge", "CAT tools proficiency"]
  },
  {
    id: 5,
    company: "LegalLingo",
    ...
    status: "Active",
    description: "Provide interpretation services for legal proceedings, client meetings, and document review sessions.",
    requirements: ["Legal interpreting certification", "Court interpreting experience", "Confidentiality protocols knowledge"]
  }
];
          ...
          <section className={styles.controlsSection}>
            <div className={styles.controls}>
              <div className={styles.filterGroup}>
                <label className={styles.filterLabel}>{t("applications.filterByStatus")}:</label>
                <select 
                ...
                >
                  ...
                  <option value="under review">{t("applications.status.underReview")}</option>
                  <option value="shortlisted">{t("applications.status.shortlisted")}</option>
                  <option value="interview scheduled">{t("applications.status.interviewScheduled")}</option>
                  <option value="rejected">{t("applications.status.rejected")}</option>
                </select>
              </div>
              
              <div className={styles.filterGroup}>
                <label className={styles.filterLabel}>{t("applications.sortBy")}:</label>
                <select 
                  value={sortBy} 
                  onChange={(e) => setSortBy(e.target.value)}
                  className={styles.filterSelect}
                >
                  <option value="newest">{t("applications.sort.newest")}</option>
                  <option value="oldest">{t("applications.sort.oldest")}</option>
                  <option value="company">{t("applications.sort.company")}</option>
                </select>
              </div>
            </div>
          </section>
                ...
                <div key={application.id} className={styles.applicationCard}>
                  <div className={styles.cardHeader}>
                    <div className={styles.companyInfo}>
                      <span className={styles.companyLogo}>{application.logo}</span>
                      <div>
                        <h3 className={styles.companyName}>{application.company}</h3>
                  ...
                  <div className={styles.cardContent}>
                    <div className={styles.jobDetails}>
                      <span className={styles.jobType}>{application.jobType}</span>
                      <span className={styles.workType}>{application.workType}</span>
                    ...
                    <div className={styles.dateApplied}>
                      {t("applications.appliedOn")}: {formatDate(application.dateApplied)}
                    </div>
                  </div>
            ...
            <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
              ...
              <div className={styles.modalBody}>
                <div className={styles.companySection}>
                  <span className={styles.modalCompanyLogo}>{selectedApplication.logo}</span>
                  <div>
                    <h3>{selectedApplication.company}</h3>
                    <p className={styles.modalLocation}>{selectedApplication.location}</p>
                  </div>
                </div>
                ...
                <div className={styles.modalDetails}>
                  ...
                  <div className={styles.detailGroup}>
                    ...
                    <span className={`${styles.modalStatus} ${getStatusClass(selectedApplication.status)}`}>
